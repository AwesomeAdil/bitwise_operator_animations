...
for i in range(n):
  Multiple_Squares(50,"black", -400 + 50 * i, "red" if int(mask[i]) else "gray", sys.argv[1][i])
...
for i in range(n):
    vivax.setx(vivax.position()[0] + 7)
    vivax.write(sys.argv[1][i] if (int(mask[i]) ^ int(sys.argv[1][i])) else "0" , font=("Verdana", 50, "normal"))
    vivax.setx(vivax.position()[0] - 7)
    vivax.setx(vivax.position()[0] + 50)
    res += sys.argv[1][i] if (int(mask[i]) ^ int(sys.argv[1][i])) else "0"
